...
# strength_tests.csv is MANUALLY maintained (not generated by this script) —
# the athlete edits it directly via GitHub whenever a strength/power/durability
# test happens (roughly every 16-week block). Loaded here so its most-recent-
# vs-prior deltas can feed the evidence catalog, same pattern as every other
# evidence category — deltas only appear once 2+ dated entries exist for a
# given exercise, so a freshly-seeded single baseline produces nothing yet.
strength_test_history = {}  # exercise -> list of {date, value, unit, category}
...
def build_evidence_catalog():
    ...
    # Strength/power/durability test deltas (from manually-maintained
    # strength_tests.csv). Only produces an item once an exercise has 2+
    # dated entries — a freshly-seeded single baseline contributes nothing
    # until the next retest, which is expected, not a bug. Percentage-based
    # threshold since units vary per exercise (kg, reps, seconds, meters) and
    ...
    for exercise, history in strength_test_history.items():
        ...
        arrow = _trend_arrow(delta_val := (latest_val - prior_val), threshold=0.01)
